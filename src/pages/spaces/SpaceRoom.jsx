...
function SpaceRoom() {
  ...
  const loadSpacePosts = useCallback(async () => {
    if (!spaceId) return
    try {
      const { data } = await supabase
        .from("posts")
        .select("id, content, author_id, created_at, media_url, media_type, profiles:author_id ( full_name, username, avatar_url )")
        .eq("space_id", spaceId)
        .order("created_at", { ascending: false })
        .limit(50)
      setSpacePosts(data || [])
    } catch (err) {
      console.warn("Failed to load space posts:", err)
    }
  }, [spaceId])
      ...
      >
        ...
        <main className="flex-1 overflow-y-auto px-4 pb-6 pt-4">
          ...
          <div className="mt-6">
            ...
            {spacePosts.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-8 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Be the first to share something</p>
              </div>
            ) : (
              <div className="space-y-3">
                {spacePosts.map((post) => {
                  const an = post.profiles?.full_name || post.profiles?.username || "MEC Member"
                    ...
                    <div key={post.id} className="rounded-2xl border border-gray-200 bg-white p-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-[#F1E7CC] text-xs font-bold text-[#A8873F]">
                          {av ? <img src={av} alt="" className="h-full w-full object-cover" /> : an.charAt(0).toUpperCase()}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-semibold text-gray-900">{an}{mine ? " · You" : ""}</p>
                          <p className="text-[10px] text-gray-400">
                            {new Date(post.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                          </p>
                        </div>
                      </div>
                      <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-gray-800">{post.content}</p>
                    </div>
                  )
                })}
              </div>
          ...
          <div className="mt-6">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
                Posts in this Space · {spacePosts.length}
              </p>
            </div>
